...
object FFileUtils {
    ...
    fun storageToal(filePath:File):Long {
        ...
        val blockSize = stat.blockSize.toLong() // 获取block的size
        val totalBlocks = stat.blockCount.toLong() // 获取block的总数
    ...
    fun storageUse(filePath:File):Long {
        val stat = StatFs(filePath.path) // 创建StatFs对象
        val availableBlocks = stat.availableBlocks.toLong() // 获取可用块大小
        val blockSize = stat.blockSize.toLong() // 获取block的size
        val totalBlocks = stat.blockCount.toLong() // 获取block的总数
     ...
     * file 文件路径
     */
    @Throws(IOException::class) fun file2byte(file:File?):ByteArray? {
    ...
    fun fileNameFilter(file:File, filterName:String?):Array<File>? {
        return if (!file.isDirectory) {
            ...
            if (pathname.name.contains(filterName!!)) {
                true
            } else {
                false
            }
