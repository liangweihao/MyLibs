...
public class JmmDNSImpl implements JmmDNS, NetworkTopologyListener, ServiceInfoImpl.Delegate  {
    ...
    public JmmDNSImpl() {
        ...
        networkChecker = new NetworkChecker(this, NetworkTopologyDiscovery.Factory.getInstance());
    ...
    public void close() throws IOException {
        if (_isClosing.compareAndSet(false, true)) {
            ...
            _timer.cancel();
            _listenerExecutor.shutdownNow();
            _jmDNSExecutor.shutdownNow();
            ...
            Lg.i(TAG,"Close JmDNS List time %s:ms",System.currentTimeMillis() - startTime);
            _knownMDNS.clear();
            _services.clear();
    ...
    public void requestServiceInfo(final String type, final String name, final boolean persistent, final long timeout) {
        // We need to run this in parallel to respect the timeout.
        for (final JmDNS mDNS : this.getDNS()) {
            if (!_jmDNSExecutor.isShutdown() && !_jmDNSExecutor.isTerminated()) {
    ...
    private static final String TAG = "JmmDNSImpl";
