...
SharedTexture::SharedTexture(AHardwareBuffer *buffer, int width, int height)
  ...
  LOGD("JNI - SharedTexture(%d, %d)", width, height);
}

SharedTexture::~SharedTexture() {
  LOGD("JNI - ~SharedTexture");
...
bool SharedTexture::bindTexture(unsigned int texId, jint target) {
  if (!AVAILABLE) {
  ...
  glext::glEGLImageTargetTexture2DOES(target, (GLeglImageOES) eglImage_);
