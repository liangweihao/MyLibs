...
package com.inair.inairsharetexture;
...
public class SharedTexture {
    ...
    public boolean bindTexture(int texId, int target) {
        if (nativeContext != 0) {
            return bindTexture(nativeContext, texId,target);
        }
        return false;
    }
    ...
    private native boolean bindTexture(long ctx, int texId,int target);
